...
public static void main(String args[]) {
	Document doc;
	try {
		doc = Jsoup.connect("http://leloi.phuyen.edu.vn/sinh-hoat-chuyen-mon/to-su-gdcd/lich-su-viet-nam-qua-cac-trieu-dai-2879-tcn-1976-.html").get();
	} catch (IOException e) {
	...
	ArrayList<ThoiKy> periods = new ArrayList<ThoiKy>();
	...
	//thoiKyException.setThuDo();
	thoiKyException.setNguon(7);
	periods.add(thoiKyException);
	...
	for (int i = 0; i < cot1.size(); i++) {
		...
		Elements boxcot1 = cot1.get(i).select("p span span");
			...
			(i == 0 || i == 8 || i == 15 || i == 19 || i == 21 || i == 24) continue;
			if (i != 3 && i != 4 && i != 6 && i != 9 && i != 16 && i != 19 && i != 20 && i != 25 && i != 26)
				for (int k = 1; k < boxcot1.size(); k++) leaders.add(boxcot1.get(k).ownText()); 
			else if (i == 6) 
			...
			for (int m = 0; m < leaders.size(); m++) {
				leaders.set(m, leaders.get(m).substring(leaders.get(m).indexOf(".") + 2));
				leaders.set(m, leaders.get(m).trim());
				if (leaders.get(m).substring(leaders.get(m).indexOf(".") + 1) == "") {
					leaders.remove(m);
	        ...
	        for (int k = 0; k < boxcot4.size(); k++)
	        	if (boxcot4.get(k).ownText().replaceAll("Giao Châu", "") != "")
	...
	String outputFile = "file\\period-source-7.json";
