...
public class ThoiKyScraper {
		...
		while(i < rows.size()-1) {
			...
			if(!row1.isEmpty()) {
				...
				if(str.length()==0) {
				    ...
				    if(part1.length()!=0) thoiKy.setTen(part1);
					if(part2.length()!=0) thoiKy.setThoiGian(part2);
				} else {
					if(str.length()!=0) thoiKy.setTen(str);
				}		
				
				String lanhDao = row.select("td").get(2).text();
				String triVi = row.select("td").get(3).text();
				if(lanhDao.length()!=0 && triVi.length()!=0) lanhDaoQuocGia.add(lanhDao+" ("+triVi+")");
				else {
					String time = row.select("td").get(3).text();
					if(time.length() != 0) thoiKy.setThoiGian(time);
				}
				while(true) {
					i++;
				...
				if(!lanhDaoQuocGia.isEmpty()) thoiKy.setLanhDaoQuocGia(lanhDaoQuocGia);
			}
	...
	public static void main(String[] args) {
		...
		try {
			doc = Jsoup.connect("http://vietycotruyen.com.vn/cac-trieu-dai-viet-nam-qua-tung-thoi-ky-lich-su?fbclid=IwAR1Ipoo1lU0ZVcOCgcWcS7eYWkbVQaRpcCLBoXH_BkXpouCIAr_63JH3ffs").get();
		} catch (IOException e) {
