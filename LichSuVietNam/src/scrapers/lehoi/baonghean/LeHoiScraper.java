		...
		for (Element title : titles) {
			...
			if (str.matches(".*\\d+\\..*")) {
				...
				if (ten.length() != 0)
					leHoi.setTen(ten);
				String anh = doc
						.select("p:contains(" + ten + ") ~ p:has(img):not(p:contains(" + ten + ") ~ p:has(strong) ~ p)")
						.select("img[src~=(?i)\\.(png|jpe?g|gif)], div.thumbnail img[src~=(?i)\\.(png|jpe?g|gif)]")
						.attr("src");
				if (anh.length() != 0)
					leHoi.setAnh(anh);
				String thoiGian = doc.select("p:contains(" + ten + ") ~ p:contains(Thời gian:):not(p:contains(" + ten
						+ ") ~ p:has(strong) ~ p)").text();
				if (thoiGian.length() != 0) {
					parts = thoiGian.split("\\:");
					leHoi.setThoiGian(parts[1].trim());
				}
				String diaDiem = doc.select("p:contains(" + ten + ") ~ p:contains(Địa điểm:):not(p:contains(" + ten
						+ ") ~ p:has(strong) ~ p)").text();
				if (diaDiem.length() != 0) {
					parts = diaDiem.split("\\:");
...
//				}
	...
	public static void main(String[] args) {
		try {
			doc = Jsoup.connect("https://baonghean.vn/nghe-an-se-to-chuc-29-le-hoi-trong-nam-2023-post263070.html")
					.get();
		} catch (IOException e) {
		...
		String outputFile = "file\\festival-source-8.json";
